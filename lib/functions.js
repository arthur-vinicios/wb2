...
const simih = async (text) => {
	try {
		const sami = await fetch(`https://st4rz.herokuapp.com/api/simsimi?kata=${text}&lang=pt`, {method: 'GET'})
		const res = await sami.json()
		if (res.result.includes("Limit 50 queries per hour.")){
		const samir = await fetch(`https://api.simsimi.net/v1/?text=${text}&lang=pt`, {method: 'GET'})
		const rese = await samir.json()
		return rese.success
		}
		if (res.result.includes("Bieber")){
		return "Albion Desenvolvedor está ativo!"
		}
		return res.result
...
const banner = cfonts.render(('Albion|Desenvolvedor|1.62'), {
    font: 'chrome',
    color: 'candy',
    align: 'center',
