   ...
   case 'pika':
		 ...
		 global.totalchatgruoup
			...
			const ing = fs.readFileSync('./assets/menu.jpg')
					client.sendMessage(from,  help(prefix, sender,getLevelingLevel(sender), namebot, user, apoia1, blocked, date, time,nameuser), text, {quoted: mek});
					
					const gttss = require('./lib/gtts')(args[0])
				
					dtt = `Olá ${nameuser} me chamo ${namebot} aqui está: minhas funções, leia com atenção para saber o que  eu posso fazer por você, caso o menu não seja enviado: pedir novamente por gentileza.`
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
...
break
                                               ...
                                               if (!daftor.includes(from)) return reply('o registro continua atio')
...
if (args.length < 1) return reply(`Parâmetro incorreto \nCommand : ${prefix}daftar nome|idade\nContoh : ${prefix}daftar Albio|23`)
					...
					case 'rg':	
					client.updatePresence(from, Presence.composing)
					if (duf == 'n')  { reply('correção aplicada')
					  
      console.log('novo registro')
const ups = {numero: sender.split('@')[0], name: "Não registrado", idade: "Não registrado",time1: time,date1: date,contype: "Free",desc: "Não registrado" }
                                                users.push(ups)
						fs.writeFileSync('./database/json/users.json', JSON.stringify(users))
